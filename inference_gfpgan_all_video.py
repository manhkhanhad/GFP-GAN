...
def main():
    ...
    parser.add_argument(
        ...
        help='Input video frame folder. Default: inputs/whole_imgs')
    ...
    for video_name in tqdm(video_list):
        ...
        elif args.version == "v1_finetune_256":
            arch = 'original_finetune_256'
            channel_multiplier = 1
            model_name = 'GFPGANv1'
            url = 'https://github.com/TencentARC/GFPGAN/releases/download/v0.1.0/GFPGANv1.pth'
        else:
        ...
        for i in range(0, len(img_list), batch_size):
            ...
            for img_path in batch_paths:
                try:
                    ...
                    batch_names.append((basename, ext))
            ...
            # try:
            if "finetune" in arch:
                # Process batch for finetune model
                restored_batch, input_batch = restorer.enhance_full_frame(
                    batch_imgs,
                ...
                # Save results
                for restored_img, input_img, (basename, _) in zip(restored_batch, input_batch, batch_names):
                ...
                # Process batch for other models
                results = [restorer.enhance(
                    img,
                    has_aligned=args.aligned,
                    only_center_face=args.only_center_face,
                    paste_back=True,
                    weight=args.weight) for img in batch_imgs]
                
                # Save results
                for (cropped_faces, restored_faces, restored_img), (basename, ext) in zip(results, batch_names):
                    # Save faces
        ...
        end_time = time.time()
        print(f"FPS: {len(img_list) / (end_time - begin_time)}")
